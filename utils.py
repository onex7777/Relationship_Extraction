...
def seq_gather(x):
    seq, idxs = x
    idxs = K.cast(idxs, 'int32') # 【batch，1】
    batch_idxs = K.arange(0, K.shape(seq)[0])
    batch_idxs = K.expand_dims(batch_idxs, 1)
    idxs = K.concatenate([batch_idxs, idxs], 1)
    return tf.gather_nd(seq, idxs)  # 得到每个batch对应sub首位置的权值【batch，dim】


def seg_padding(batch,max_length=256, padding=0,):

    x =np.array([
        np.concatenate([batch, [padding] * (max_length - len(batch))]) if len(batch) < max_length else batch[:max_length]])
    return x
...
def extract_items(subject_model, object_model, tokenizer, text_in, id2rel, h_bar=0.5, t_bar=0.5):
    ...
    # token_ids = seg_padding(token_ids)
    # segment_ids = seg_padding(segment_ids)
    ...
    sub_heads_logits, sub_tails_logits = subject_model([token_ids, segment_ids])  # 使用的使subject_model模型，能预测出subject
    # 若条件满足，返回索引值

    sub_heads, sub_tails = np.where(sub_heads_logits[0] > h_bar)[0], np.where(sub_tails_logits[0] > t_bar)[0]  # 使用阈值
    # print(type(sub_heads), sub_heads, sub_tails)
    ...
    if subjects:
        triple_list = []
        token_ids = np.repeat(token_ids, len(subjects), axis=0)
        segment_ids = np.repeat(segment_ids, len(subjects), axis=0)
        sub_heads, sub_tails = np.array([sub[1:] for sub in subjects]).T.reshape((2, -1, 1))
        obj_heads_logits, obj_tails_logits = object_model([token_ids, segment_ids, sub_heads, sub_tails])
...
def metric(subject_model, object_model, eval_data, id2rel, tokenizer, exact_match=False, output_path=None, name=None):
    if output_path:
        F = open(output_path, 'w', encoding='utf8')
    orders = ['subject', 'relation', 'object']
    correct_num, predict_num, gold_num = 1e-10, 1e-10, 1e-10
    for line in tqdm(iter(eval_data), total=len(eval_data), desc=name):
